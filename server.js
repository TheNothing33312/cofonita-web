...
if (fs.existsSync(envPath)) {
    ...
    console.log('⚠️  No se encontró .env en la raíz, usando .env local');
...
const {
    ...
    NODE_ENV = 'production',
    // URLs importantes
    WEBSITE_URL = 'https://cofonitabot.netlify.app',
    API_URL = process.env.API_URL || 'https://cofonitabot.onrender.com', // Cambia esto por tu URL real del backend
    // Para desarrollo local
    IS_LOCAL = process.env.IS_LOCAL || 'false'
} = process.env;

// Determinar URLs basadas en el entorno
const isLocal = IS_LOCAL === 'true';
const isProduction = NODE_ENV === 'production';

// URLs finales
let FINAL_WEBSITE_URL = WEBSITE_URL;
let FINAL_API_URL = API_URL;
let FINAL_REDIRECT_URL = `${FINAL_API_URL}/auth/discord/callback`;

// En desarrollo local, ajustar URLs
if (isLocal) {
    FINAL_API_URL = `http://localhost:${PORT}`;
    FINAL_REDIRECT_URL = `${FINAL_API_URL}/auth/discord/callback`;
...
// Validar que el callback URL sea accesible desde internet (solo en producción)
if (isProduction && !isLocal && (FINAL_REDIRECT_URL.includes('localhost') || FINAL_REDIRECT_URL.includes('127.0.0.1'))) {
    console.error('❌ ERROR: REDIRECT_URL no puede ser localhost en producción');
    console.error('   Configura API_URL en tus variables de entorno con tu dominio real del backend');
    process.exit(1);
}
...
app.use(session({
    ...
    cookie: {
        secure: isProduction && !isLocal, // true solo en producción con HTTPS
        maxAge: 7 * 24 * 60 * 60 * 1000, // 7 días
        httpOnly: true,
        sameSite: isProduction && !isLocal ? 'lax' : 'lax'
    },
    name: 'cofonita.sid',
...
passport.use(new DiscordStrategy({
    ...
    callbackURL: FINAL_REDIRECT_URL, // Usar la URL final
...
// Middleware para CORS (importante para que el frontend pueda comunicarse)
app.use((req, res, next) => {
    // Permitir solicitudes desde tu frontend de Netlify
...
// Middleware para asegurar conexiones seguras en producción
app.use((req, res, next) => {
...
app.use((err, req, res, next) => {
    ...
    const errorResponse = {
        error: 'Error interno del servidor',
        message: NODE_ENV === 'development' || isLocal ? err.message : 'Algo fue mal bro, estoy sad. Diselo a Salvox',
        timestamp: new Date().toISOString()
    };
...
app.listen(serverPort, '0.0.0.0', () => {
    ...
    console.log(`📌 Entorno: ${NODE_ENV.toUpperCase()} ${isLocal ? '(LOCAL)' : ''}`);
    console.log(`🌐 Frontend (Netlify): ${FINAL_WEBSITE_URL}`);
    console.log(`🔧 Backend (Render/Heroku): ${FINAL_API_URL}`);
    ...
    console.log(`   2. Selecciona tu aplicación Cofonita`);
    console.log(`   3. Ve a OAuth2 > General`);
    console.log(`   4. En "Redirects", añade: ${FINAL_REDIRECT_URL}`);
    console.log('='.repeat(60));
